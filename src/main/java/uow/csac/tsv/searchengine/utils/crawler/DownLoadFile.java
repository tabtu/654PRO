	...
	public final String downloadFile(final String url) {
		...
		HttpRequestRetryHandler myRetryHandler = new HttpRequestRetryHandler() {
			...
			public boolean retryRequest(final IOException arg0, final int executionCount, final HttpContext arg2) {
				if (executionCount > Config.MAX_RETRY_TIMES) {
					return false;
				}
				return false;
			}
		...
		CloseableHttpClient httpClient = HttpClients.custom().setRetryHandler(myRetryHandler).build(); // 生成
																										// HttpClinet
		RequestConfig requestConfig = RequestConfig.custom().setConnectTimeout(Config.DEFAULT_CONNECT_TIMEOUT)
				.setConnectionRequestTimeout(Config.DEFAULT_CONNECTION_REQUEST_TIMEOUT).build();
		...
		try {
			...
			if (entity != null) { // 处理流内容
				...
				filePath = Path.DATA_PATH + "html/" + getFileNameByUrl(url);
